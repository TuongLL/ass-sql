...
--Don hang chi co 2 trang thai 'da giao hang' va 'chua giao hang'
GO
create or alter trigger tinhtrangdonhang
ON donhang
FOR INSERT, UPDATE
AS 
BEGIN
	IF EXISTS(SELECT * FROM INSERTED WHERE Trangthai NOT LIKE 'Da giao hang')
	BEGIN
		raiserror (N'Lỗi: Chi co 2 trang thai Da giao hang va Chua giao hang', 16,1)
		rollback
	END
	IF EXISTS(SELECT * FROM INSERTED WHERE Trangthai NOT LIKE 'Chua giao hang')
	BEGIN
		raiserror (N'Lỗi: Chi co 2 trang thai Da giao hang va Chua giao hang', 16,1)
		rollback
	END
